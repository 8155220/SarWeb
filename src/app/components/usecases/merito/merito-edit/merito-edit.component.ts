...
@Component({
  selector: "app-merito-edit",
  templateUrl: "./merito-edit.component.html",
  styleUrls: ["./merito-edit.component.scss"]
})
export class MeritoEditComponent implements OnInit {
  persona: any;
  formGroup: FormGroup;
  loading = true;
  meritoid='';
    ...
    this.activatedRoute.params
      .pipe(
        ...
        map(e => {
          this.meritoid=e['id'];
          return e["id"]})
        ,switchMap(id=> this.meritoService.getMerito(id).pipe(first()))
      )
      .subscribe((e:any) => {
        this.loading=false;
        this.persona=e;
  ...
  onSubmit() {
    ...
    console.log(this.meritoid);
    
    

    this.meritoService.updateMerito(this.formGroup.value,this.meritoid).subscribe(
      e => {
        this.loading = false;
